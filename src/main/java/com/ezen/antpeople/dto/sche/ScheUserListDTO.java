...
public class ScheUserListDTO {
	...
	//직원 일정 상태를 같이 String으로 변환
	public String toStringStaff() {
		//스케줄 생성 함수
		String schedules = scheduleString(this.scheduleList);
		String users = StaffString(this.userToScheList);
		log.info(users);
		if(users != "")
			return "{\""+this.month+"\":"+schedules+","+users+"}";
		else
	...
	public Map<String, Set<String>> userScheduleList(Set<ScheUserDTO> scheduleList){
		...
		public String StaffString(Set<ScheUserDTO> userToScheList) {
			Optional<Map<String, Set<Map<String,Integer>>>> userSchedule = 
					Optional.of(staffScheduleList(userToScheList));
			String users = "";
			if(!userSchedule.get().isEmpty()) {
				log.info("일정에 대한 사용자 존재");
				for(String user_id : userSchedule.get().keySet()) {
					users += "\""+user_id+"\":{";
					for(Map<String,Integer> schedule : userSchedule.get().get(user_id)) {
						for(String key : schedule.keySet())
							users +="\""+key +"\":{\"state\":\""+schedule.get(key)+"\"},";		
					}
					users = users.substring(0, users.length()-1) +"} ,";
				}
				users = users.substring(0, users.length()-1);
	...
	//직원별 신청 일정 리스트 만들기
		public Map<String, Set<Map<String,Integer>>> staffScheduleList(Set<ScheUserDTO> scheduleList){
			log.info("일정 리스트 : " + scheduleList);
			Map<String, Set<Map<String,Integer>>> userSchedule = new HashMap<String, Set<Map<String,Integer>>>();
			
			for(ScheUserDTO schedule : scheduleList) {
				Optional<UserDetailDTO> user = Optional.ofNullable(schedule.getUser());
				String scheduleId = schedule.getUnique(); //일정의 아이디
				Integer scheduleState = schedule.getState(); //일정의 상태
				if(user.isPresent()) {
					Map<String,Integer> newSchedule = new HashMap<String,Integer>();
					newSchedule.put(scheduleId, scheduleState);
					Set<Map<String,Integer>> schedules = new HashSet<Map<String,Integer>>();
					String key = user.get().getUser_id()+"_"+user.get().getName();
					if(userSchedule.get(key) != null) {
						log.info("기존의 등록된 정보가 있는 회원");
						schedules = userSchedule.get(key);
