...
<<<<<<< HEAD
	 ...
	 * public String logincheck(@RequestParam("email") String
	 * email, @RequestParam("password") String password) throws Exception {
	 * logger.info("체크 페이지"); // public Model logincheck(HttpServletRequest request,
	 * Model model) throws Exception { UserDTO userDto = new UserDTO();
	 * userDto.loginUser(email, password); String returnURL ="";
	 * if(userService.verifyPassword(userDto)) { logger.info("확인 성공 / 사용자 구분시작");
	 * returnURL = "/common/notice"; } else { returnURL = "/pages/login"; } return
	 * returnURL; }
	 * 
	 * @RequestMapping("pages/register") public String register() throws Exception {
	 ...
	 * userService.saveUser(userDto); return "pages/login"; }
	 * 
	 * // 출퇴근 기능
	 * 
